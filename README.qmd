...
The `predict_response(interval = "prediction")` function plots the prediction interval separately for each predictor, displaying uncertainty around where the data may lay around the model.
...
`pp_check(type = "scatter_avg")` shows the model predictions against the data as a scatterplot. Having all of the points along the line would indicate very good fit.

```{r}
pp_check(m.crab.lat.air.water, type = "scatter_avg")
```

Here, it seems that there is a lot of variation at each site: the model line goes through approximately the middle of each cloud of points, which is good, but there is still a lot of unexplained variation at seemingly the site level.
